...
//    if(d>0.01) d =0.01; ///for FPS count - to get frame ticks Game class connects "&QGraphicsScene::changed" with "this, &Game::frameHandler" - so it calls frameHandler when somethign actually FIRST changes in the in the scene, thus to avoid getting suddent movement of speed value 200, i artificially set duration to 0.001 to limit speed in the first frame
if(!is_sceneChanged) return; ///first change in the scene, so we ll pass it, cause delta time is too high, close to 1, so speed would be 295
